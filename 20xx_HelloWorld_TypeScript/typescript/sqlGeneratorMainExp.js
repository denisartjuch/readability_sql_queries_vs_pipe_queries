...
class Query {
    ...
    clone() {
        ...
        if (this.errorMarker) {
            q.errorMarker = Object.assign({}, this.errorMarker);
        }
        return q;
    }
}
...
const STATIC_WORDS = [
    ...
    "porch", "fresh", "lager", "flint", "scarf", "tonic", "cough", "litre", "fiver", "attic", "creed", "cocoa", "weber", "goose", "jelly", "greed",
    ...
    "shout", "acres", "ethos", "ryder", "frown", "satin", "bream", "truce", "spark", "niche", "aisle", "locus", "grill", "forth", "beech", "screw",
    ...
    "prior", "siren", "proud", "chang", "dummy", "alert", "scrub", "shoot", "bosom", "forge", "smash", "acorn", "xerox", "logan", "lapse", "denim",
    ...
    "brunt", "roche", "pixel", "facet", "jetty", "gable", "toxin", "leone", "crush", "optic", "vined", "knack", "moray", "strat", "opium", "poker",
    ...
    "lionel", "garvey", "uplift", "marrow", "argyll", "tiller", "karate", "beacon", "anthem", "saucer", "plough", "bunker", "crunch", "raffle", "worthy",
...
function isValidTriplet(att, agg, group) {
    if (att + agg === 0)
        return false;
...
function precomputeAllValidTripletSequences(numberQueries, totalCosts) {
    const remainingCosts = totalCosts - BASE_COLUMNS;
    if (remainingCosts <= 0)
        return [];
    const costPartitions = integerPartitionsWithConstraint(remainingCosts, numberQueries, (n) => n > 1);
    const allValidSequences = [];
    for (const costPartition of costPartitions) {
        const tripletOptionsPerQuery = [];
        ...
        if (tripletOptionsPerQuery.length !== numberQueries)
            continue;
...
function isValidSequence(sequence) {
    ...
    for (const [att, agg, group] of sequence) {
        ...
        if (agg > 0 && aggregatableCols <= 0)
            return false;
        if (agg > aggregatableCols * AGG_FUNCTIONS.length)
            return false;
...
function injectError(root, errorType, errorCost, nounPool) {
    ...
    if (chain.length === 0)
        return null;
    const baseNouns = new Set(chain[0].attributes);
    let costCounter = 0;
    const generateNewNoun = () => {
    ...
    for (const q of chain) {
        switch (errorType) {
            ...
            case "AGG_UNKNOWN": {
                ...
                for (let i = 0; i <= q.aggregates.length; i++) {
                    if (costCounter + 1 === errorCost) {
                        if (q.attributes.length > 0 && q.group_by.length === 0)
                            return null;
...
function computeErrorCostPipe(root) {
    const chain = getQueryChain(root);
    let costCounter = 0;
    for (const q of chain) {
    ...
    for (const q of chain) {
        if (q.errorMarker) {
            switch (q.errorMarker.role) {
                case "ATTR": return (_a = q.attributes[q.errorMarker.index]) !== null && _a !== void 0 ? _a : null;
                case "GROUP": return (_b = q.group_by[q.errorMarker.index]) !== null && _b !== void 0 ? _b : null;
                case "AGG": return (_d = (_c = q.aggregates[q.errorMarker.index]) === null || _c === void 0 ? void 0 : _c.attribute) !== null && _d !== void 0 ? _d : null;
            }
        }
    }
    return null;
...
function buildQueryFromTripletSequence(tripletSequence, nounPool) {
...
function generateQuery(precomputedSequences, errorType, errorCost, desiredDiff, totalCosts) {
    var _a, _b;
    ...
    for (const idx of shuffledIndices) {
        ...
        if (!root)
            continue;
        if (root.total_costs_CTE() !== totalCosts)
            continue;
        ...
        if (root.total_costs_CTE() !== expectedCost)
            continue;
        ...
        return {
            ...
            columnsBaseQuery: (_b = (_a = chain[0]) === null || _a === void 0 ? void 0 : _a.attributes.length) !== null && _b !== void 0 ? _b : BASE_COLUMNS,
            cteErrorLine,
            pipeErrorLine
        };
    }
...
for (const et of errorTypes) {
    for (const diffTarget of desiredDiffs) {
        ...
        while (produced < repetitionsPerCombo && localTries < triesLimitPerCombo) {
            localTries++;
            const gen = generateQuery(precomputedSequences, et, et === "NoError" ? undefined : errorCost, diffTarget, totalCosts);
            if (gen != null) {
                resultArray.push({
                    sqlQuery: gen.sql,

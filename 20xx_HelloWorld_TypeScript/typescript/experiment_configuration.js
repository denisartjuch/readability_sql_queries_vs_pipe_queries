...
import { formatted } from "../queries100RepsMainExperiment.js";
...
FROM step_1;</code></pre>`);
        ...
        pre_run_training_instructions: writer.string_page_command(writer.convert_string_to_html_string("You have entered the training phase. It consists of 9 example queries per syntax.\n\nFor each query, decide whether it contains a syntactic error:\n- If an error is present, enter the line number where the error occurs (or where you first notice it).\n- If there is no error, press 0 and [ENTER] to continue.\n\nYou will advance to the next task after you enter your answer.")),
        pre_run_experiment_instructions: writer.string_page_command(writer.convert_string_to_html_string("You entered the experiment phase. It should take probably 15-30 minutes until the end of experiment.")),
        post_questionnaire: [

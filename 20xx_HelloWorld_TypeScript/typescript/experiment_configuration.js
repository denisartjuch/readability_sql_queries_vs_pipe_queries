...
import { Time_to_finish, text_input_experiment, SET_SEED, alternatives, } from "../../N-of-1-Experimentation/modules/Experimentation/Experimentation.js";
import { formatted } from "./../queries20RepsMainExp.js";
        ...
        introduction_pages: [
          ...
          <section class="instructions" lang="en">
            ...
            <p>
              ...
              (<strong>approximately 15–25 minutes</strong>), are able to focus, and feel motivated to complete it in one go.
            </p>
            ...
            <h2>Your Task</h2>
            <p>
              In each trial, you will be shown <strong>two SQL queries</strong> with different syntaxes:
            </p>
            <ul>
              <li><strong>Standard SQL syntax</strong>, or</li>
...
-- Visible columns after this step: customer_id, order_total, order_date, revenue, avg_order_value
|&gt; AGGREGATE
...
`);
            ...
            () => {
                writer.print_string_on_stage(`<h2>Pipe example 3: aggregation (GROUP BY) with sample table</h2>
<p>
  This example uses aggregation: we group by <span class="sourcecode">city</span>,
  count people, and compute the average age. Then we sort the summary.
</p>
...
<h3>SQL example 1: projection + derived columns</h3>
...
WITH step_0 AS (
  SELECT
   ...
   GROUP BY
    ...
    order_total
  FROM base
  -- Visible columns after step_0: customer_id, order_id, order_total, sum_order_ts
),
step_1 AS (
  SELECT
    customer_id,
    ...
    order_total
  FROM step_0
  -- Visible columns after step_1: customer_id, sum_order_ts, order_total
)
SELECT *
...
`);
            ...
            () => {
                writer.print_string_on_stage(`<h2>SQL example 3 (CTEs only): Aggregation + GROUP BY</h2>

<p>
  Same logic as the pipe version, but written as plain SQL using CTEs:
  group by <span class="sourcecode">city</span>, count people, compute average age, and sort.
</p>
...
<div style="display:flex; gap:14px; align-items:flex-start; width:100%;">
  <div style="flex:1 1 50%; min-width:0;">
    <h4>Pipe Syntax</h4>
    <pre class="sourcecode"><code>FROM orders
...
<p>
  A column is projected in the <strong>SELECT</strong> clause even though it has not been projected or aggregated earlier.
...
|&gt; SELECT
    sponge,
    trees,
    crown   -- ❌ Error Type 1: "crown" appears for the first time in SELECT and was not part of the table base
|&gt; SELECT *;</code></pre>
...
CTE_1 AS (
  SELECT
    sponge,
    trees,
    crown   -- ❌ Error Type 1:  "crown" appears for the first time in SELECT of CTE_1 and was not part of the table base/CTE_0
  FROM CTE_0
)
SELECT *
FROM CTE_1;</code></pre>
...
<p>
  A column appears in <strong>GROUP BY</strong> even though it has not appeared previously
  (i.e., it was neither projected nor aggregated before).
</p>
<p>
...
|&gt; AGGREGATE
   ...
   GROUP BY
     sponge,
     expert,
     crown   -- ❌ Error Type 2: "crown" was never projected in the table base
|&gt; SELECT *;</code></pre>
...
CTE_1 AS (
  ...
  GROUP BY
    sponge,
    expert,
    crown   -- ❌ Error Type 2: crown was never projected in the CTE_0 / table base
)
SELECT *
FROM CTE_1;</code></pre>`);
            },
...
<p>
  An aggregate function references a column that has never been projected before.
  In other words, the column inside the aggregate is invalid in the current query context.
</p>
<p>
  <strong>Only occurs in:</strong>
...
CTE_1 AS (
  SELECT
    sponge,
    AVG(expert) AS avg_expert,
    SUM(crown)  AS sum_crown   -- ❌ Error Type 3: "crown" is unknown in CTE_1 and can not be aggregated
  FROM CTE_0
  GROUP BY 
    sponge
)
...
FROM CTE_1;</code></pre>`);
        ...
        pre_run_training_instructions: writer.string_page_command(writer.convert_string_to_html_string("You have entered the training phase. It consists of one example query per syntax and error type.\n\nFor each query, decide whether it contains a syntactic error:\n- If an error is present, enter the line number where the error occurs (or where you first notice it).\n- If there is no error, press [ENTER] to continue.\n\nYou will advance to the next task only after you enter the correct answer.")),
        pre_run_experiment_instructions: writer.string_page_command(writer.convert_string_to_html_string("You entered the experiment phase. It should take probably 15-20 minutes until the end of experiment.")),
        post_questionnaire: [
        ...
        layout: [
            ...
            { variable: "errorType", treatments: ["SELECT_UNKNOWN", "AGG_UNKNOWN", "GROUP_UNKNOWN", "Decoy"] },
        ...
        training_configuration: {
            fixed_treatments: [
                ["sql", "AGG_UNKNOWN", "0"],
                ["sql", "GROUP_UNKNOWN", "0"],
                ["sql", "SELECT_UNKNOWN", "0"],
                ["pipe", "AGG_UNKNOWN", "0"],
                ["pipe", "GROUP_UNKNOWN", "0"],
                ["pipe", "SELECT_UNKNOWN", "0"],
            ],
        ...
        task_configuration: (t) => {
            ...
            const task = random_array_element(results.filter((entry) => entry.totalInformationDifference === current_InformationDifference
                && entry.errorType === current_errorType));
            const queryText = current_queryKind === "pipe" ? task.pipeQuery : task.sqlQuery;
            ...
            const renderTwoColumn = (leftHtml, rightHtml) => `
        <div style="
          display:flex;
          gap:14px;
      ...
      `;
        ...
        `;
            ...
            t.do_print_after_task_information = () => {
                var _a;
                writer.clear_stage();
                (_a = writer.clear_error) === null || _a === void 0 ? void 0 : _a.call(writer);
                const correctAnswer = current_queryKind === "pipe" ? task.errorLinePipe : task.errorLineSQL;
                const attribuetsUntilError = current_queryKind === "pipe" ? task.totalInformationUntilErrorPipe : task.totalInformationUntilErrorSQL;
                ...
                const generalHint = "In case your answer was wrong:\n" +
          ...
          <div style="font-size:0.98em; line-height:1.45;">
            <p style="margin:0 0 10px 0;"><strong>The correct answer was:</strong> ${writer.convert_string_to_html_string(correctAnswerText)}</p>

            <p style="margin:0 0 16px 0;"><strong>The wrong attribute was:</strong> ${writer.convert_string_to_html_string(wrongAttribute)}.</p>

            <p style="margin:0 0 8px 0;"><strong>Hint</strong></p>
            <p style="margin:0 0 16px 0; white-space:pre-wrap;">${writer.convert_string_to_html_string(generalHint)}</p>
